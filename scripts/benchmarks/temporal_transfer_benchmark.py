#!/usr/bin/env python3
"""
temporal_transfer_benchmark.py
==============================
Fit SGS models on the original cutouts at the original time window and evaluate
them on later-time cutouts, with and without a spatial shift.
...
def main() -> None:
    np.random.seed(0)
    out_dir = "results/temporal_transfer"
    ...
    test_cases = [
        (
            "future_same_window",
            JHTDBOracle("jhtdb_u_tensor_64_future.h5", filter_width=1.0, boundary_mode="wrap"),
            JHTDBOracle("channel_u_tensor_64_future.h5", filter_width=1.0, boundary_mode="nearest"),
        ),
        (
            "future_shifted_window",
            JHTDBOracle("jhtdb_u_tensor_64_future_shifted.h5", filter_width=1.0, boundary_mode="wrap"),
            JHTDBOracle("channel_u_tensor_64_future_shifted.h5", filter_width=1.0, boundary_mode="nearest"),
        ),
    ]
    ...
    for case_name, test_iso, test_chan in test_cases:
        for model in models:
    ...
    out_path = os.path.join(out_dir, "temporal_transfer_summary.csv")
